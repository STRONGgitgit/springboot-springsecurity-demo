  ...
  <body>
	          ...
	          <div class="widget-title"> <span class="icon"><i class="icon-th"></i></span>
	            <h5>角色列表</h5>
	          	...
	          	<form class="form-inline">
	          	 <security:authorize buttonUrl="/role/addRole,POST"><%-- <security:authorize buttonUrl="/role/addRole.do">--%>
     ...
     $(document).ready(function() {
		table = $('#datatable').DataTable( {
				...
				"sPaginationType": "full_numbers",
			  	...
			  	"ajax": '${ss}/role/roleList',/*角色管理分页查询*/
			  	"columns": [
		            { "data": "id" },
		            { "data": "roleKey" },
		            { "data": "roleDesc" },
			...
			columnDefs:[{
                targets: 3,
                render: function (data, type, row, meta) {<%--<security:authorize buttonUrl='/role/delRole.do'>--%>
                    return '<p><security:authorize buttonUrl='/role/delRole/*,DELETE'><a type="button" class="btn btn-danger  btn-default" href="javascrip:;" onclick=delById(' + row.id + ') >删除</a></security:authorize> '+
                    '<security:authorize buttonUrl='/role/saveRoleResources,GET'><a type="button" class="btn btn-success btn-default" href="javascrip:;" onclick=allotResources(' + row.id + ') >分配权限</a></security:authorize></p>';
					   <%--<security:authorize buttonUrl='/role/saveRoleResources.do'>--%>
                }
            },
	...
	function allotResources(rid){
		...
		$.ajax({
			...
			type : "GET",
			data:{rid:rid},
			url: "${ss}/resources/resourcesListWithRole",/*查询资源列表，并且返回指定角色是否拥有该资源的权限*/
			dataType:'json',
