				   ...
				   <div class="form-group">
			        ...
			        <button type="button" id="btn_search" onclick="search();" class="btn btn-primary">查询</button>
		          	<%--<security:authorize buttonUrl="/user/addUser.do">--%>
						<security:authorize buttonUrl="/user/addUser,POST">
	...
	//_csrf参数设置
     ...
     $(document).ready(function() {
		table = $('#datatable').DataTable( {
			...
			columnDefs:[{
                targets: 3,
                render: function (data, type, row, meta) {<%--<security:authorize buttonUrl='/user/delUser.do'>--%>
                    return '<security:authorize buttonUrl='/user/delUser/*,DELETE'><p><a type="button" class="btn btn-danger  btn-default" href="javascrip:;" onclick=delByID(' + row.id + ') >删除</a></security:authorize> '+
                    '<security:authorize buttonUrl='/user/saveUserRoles,POST'><a type="button" class="btn btn-success btn-default" href="javascrip:;" onclick=allotRole(' + row.id + ') >分配角色</a></p></security:authorize>';
					<%--<security:authorize buttonUrl='/user/saveUserRoles.do'>--%>
                }
            },
